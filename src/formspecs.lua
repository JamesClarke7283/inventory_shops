...
function inventory_shops.get_shop_formspec(player_name, page, search_query)
    ...
    local items_per_page = 48  -- Adjusted for formspec size
        ...
        if not stack:is_empty() then
            ...
            if search_query == "" or display_name:lower():find(search_query:lower(), 1, true) or owner:lower():find(search_query:lower(), 1, true) then
                if display_index >= start_index and shown_items < items_per_page then
                    -- Add item slot to formspec (positions calculated for an 8x6 grid)
                    local col = shown_items % 8
                    local row = math.floor(shown_items / 8)
    ...
    local formspec = "formspec_version[3]" ..
                     ...
                     "field[0.3,0.6;6,1;search;;" .. minetest.formspec_escape(search_query or "") .. "]" ..
                     "button[6,0.25;2,1;search_button;Search]" ..
                     "button_exit[7.3,0.25;0.7,1;exit_button;X]" ..
                     formspec_item_list ..
                     "button[0,7;1,1;prev_page;<]" ..
                     "label[1,7.2;Page " .. page .. " of " .. total_pages .. "]" ..
                     "button[2,7;1,1;next_page;>]"
        ...
        if field_name:sub(1, 5) == "item_" then
            ...
            if index then
                ...
                if not stack:is_empty() then
                    ...
                    local shift_pressed = minetest.is_shift_pressed(fields)
                    ...
                    if player_funds >= total_price then
                        ...
                        if success ~= false then
                            ...
                            local purchase_stack = ItemStack(item_name .. " " .. purchase_count)

                            -- Remove items from shop inventory
                            local shop_stack = stack:take_item(purchase_count)
                            inv:set_stack("items", index, shop_stack)
...
-- Detect if Shift key is pressed
function minetest.is_shift_pressed(fields)
    return fields.key_pressed and fields.key_pressed:find("KEY_SHIFT")
end
